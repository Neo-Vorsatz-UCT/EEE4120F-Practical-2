...
function run_analysis_()
    ...
    %   c. Store results (image size, time_serial, time_parallel, speedup)
        ...
        diff_pixels(i) = sum(xor(plot_serial, plot_parallel), 'all'); %calculate error
        labels{i} = strcat(int2str(width),"x",int2str(height));
    ...
    %   d. Plot and save the Mandelbrot set images generated by both methods
        filepath = strcat(serial_images_dir,"/serial_",labels{i},".png");
        mandelbrot_plot(plot_serial, filepath);
        filepath = strcat(parallel_images_dir, "/parallel_", labels{i},".png");
